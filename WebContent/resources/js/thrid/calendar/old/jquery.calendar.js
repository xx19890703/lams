...
(function($) {
    $.fn.suunDateTimeReadOnly = function(options) {};
        ...
        */
		...
		(function(){
			function makeTimePart(className, init, range_start, range_end) {
				...
				//zyfang Add;
				Calendar.addEvent(part, "keyup", function(e){
					e = e || window.event;  
					el=e.srcElement||e.currentTarget; 
					for (var i = el._range.length; --i >= 0;){
						if (Number(el._range[i]) == Number(el.value)){
							cal.onUpdateTime();
                            return;
						}				        
					}					
				...
				Calendar.addEvent(part, "blur", function(e){	
					e = e || window.event;  
					el=e.srcElement||e.currentTarget; 
					for (var i = el._range.length; --i >= 0;){
						if (Number(el._range[i]) == Number(el.value)){
			...
			var secs=cal.date.getSeconds();//zyfang add
			var t12 = !cal.time24;
			var pm = (hrs > 12);
			if (t12 && pm) hrs -= 12;
			...
			//zyfang add
			var spans = Calendar.createElement("span", cell);
			spans.innerHTML = ":";
			spans.className = "colon";
			...
			if (!cal.time24){
			    ...
			    cell.colSpan = 2;
			    //zyfang add
			    var AP = Calendar.createElement("span", cell);
			    AP.className = "ampm";
			    AP.innerHTML = pm ? "pm" : "am";
			    AP.calendar = cal;
			    AP.ttip = Calendar._TT["TIME_PART"];
			    AP.navtype = 50;
			    AP._range = ["am", "pm"];
				Calendar._add_evs(AP);
			    //zyfang add
			    /* zyfang if (t12)
			...
			cal.onSetTime = function() {
				var pm, hrs = this.date.getHours(),
					...
					secs=this.date.getSeconds();//zyfang add
				if (t12) {
					pm = (hrs >= 12);
					if (pm) hrs -= 12;
					if (hrs == 0) hrs = 12;
				...
				S.innerHTML = (secs < 10) ? ("0" + secs) : secs;//zyfang add
				*/
				//H.value = (hrs < 10) ? ("0" + hrs) : hrs;
				//M.value = (mins < 10) ? ("0" + mins) : mins;
				//S.value = (secs < 10) ? ("0" + secs) : secs;//zyfang add
				H.setAttribute("value", (hrs < 10) ? ("0" + hrs) : hrs);
				M.setAttribute("value", (mins < 10) ? ("0" + mins) : mins);
				S.setAttribute("value", (secs < 10) ? ("0" + secs) : secs);
			};

			cal.onUpdateTime = function() {
				var date = this.date;
				//zyfang var h = parseInt(H.innerHTML, 10);
				...
				*/
				...
				date.setSeconds(parseInt(S.value, 10));
				date.setFullYear(y);
				date.setMonth(m);
				date.setDate(d);
				this.dateClicked = false;
...
Date.parseDate = function(str, fmt) {
	...
	if (isNaN(sec)) sec = today.getSeconds();//zyfang add
	if (y != 0 && m != -1 && d != 0)
		return new Date(y, m, d, hr, min, sec);//zyfang 0-->sec
	y = 0; m = -1; d = 0;
	for (i = 0; i < a.length; ++i) {
