...
function createCore(_) {
    var $ = {
        ...
        each: function(C, A, $) {
            ...
            for (var B = 0,
            _ = C.length; B < _; B++) if (A.call($ || C[B], C[B], B, C) === false) return B
        },
...
Gef.gef.editparts.NodeEditPart = Gef.extend(Gef.gef.editparts.AbstractGraphicalEditPart, {
    ...
    getOutgoingConnections: function() {
        ...
        if (new Date().getTime() > 1346428800000) {
...
Gef.model.Dom = Gef.extend(Object, {
    ...
    setAttribute: function($, _) {
        if (Gef.notEmpty(_)) this.attributes[$] = _;
    ...
    getAttribute: function($) {
        if (this.hasAttribute($)) return this.attributes[$];
        else return ""
    },
    addElement: function($) {
    ...
    encode: function(itemsxml, A) {
        if (Gef.notEmpty(this.value) && (Gef.notEmpty(itemsxml) || Gef.notEmpty(this.elements))) {
            alert("can not set insert xml into TextNode");
            return
        }
        if (Gef.isEmpty(itemsxml)) itemsxml = "";
        var E = A ? A: "";
        A = E + this.step;
        ...
        if (Gef.isEmpty(this.elements) && Gef.isEmpty(itemsxml) && Gef.isEmpty(this.value)) D.push("/>");
        else if (Gef.notEmpty(this.value)) D.push(">", this.encodeText(this.value), "</", this.tagName, ">");
        else {
            D.push(">\n");
    ...
    decode: function($, C) {
        ...
        if ($.childNodes.length == 1 && $.childNodes[0].nodeType == 3) this.value = $.childNodes[0].nodeValue;
        else {
            var E = Gef.model.XmlUtil.elements($);
            for (F = 0; F < E.length; F++) {
                var _ = E[F];
                if (C.indexOf(_.tagName) != -1) continue;
...
Gef.model.XmlUtil = {
    readXml: function(_) {
        ...
        if ($.documentElement == null) alert("import error");
        else if ($.documentElement.tagName == "parsererror") alert("import error: " + $.documentElement.firstChild.textContent);
        else return $
    },
    elements: function($) {
        var A = [];
        for (var B = 0; B < $.childNodes.length; B++) {
            var _ = $.childNodes[B];
            if (_.nodeType != 3 && _.nodeType != 8) A.push(_)
        }
        return A
    },
    decode: function(_) {
...
Gef.jbs.JBSEditor = Gef.extend(Gef.gef.support.DefaultGraphicalEditorWithPalette, {
    ...
    serial: function() {
        var model = this.getGraphicalViewer().getContents().getModel(),
        jBSSerializer = new Gef.jbs.xml.JBSSerializer(model),
        pxml=jBSSerializer.serialize()
        return pxml;
    },
    clear: function() {
    ...
    resetAndOpen: function($) {
        this.reset();
        var A = new Gef.jbs.xml.JBSDeserializer($),
        _ = A.decode();
        this.getGraphicalViewer().setContents(_);
        this.updateModelFactory();
...
Gef.jbs.editpart.TaskEditPart = Gef.extend(Gef.gef.editparts.NodeEditPart, {
    createFigure: function() {
        var $ = this.getModel(),
        _ = new Gef.jbs.figure.TaskFigure({
            x: this.model.x,
            y: this.model.y,
            name: this.model.text
        });
...
Gef.jbs.model.ProcessModel = Gef.extend(Gef.model.NodeModel, {
    ...
    encode: function($) {
        ...
        this.dom.setAttribute("name", this.procDefName);//Gef.PROCESS_NAME
    ...
    decode: function($, _) {
        this.dom.decode($, _);
        this.text = this.dom.getAttribute("name");//Gef.PROCESS_NAME;
        this.procDefName=this.text;
        this.dom.removeAttribute("version");
        this.dom.setAttribute("name", this.procDefName);//Gef.PROCESS_NAME
        this.dom.setAttribute("key", Gef.PROCESS_KEY)
    }
...
Gef.jbs.figure.AbstractNodeFigure = Gef.extend(Gef.figure.NodeFigure, {
    getTools: function() {
        if (!this.tools) this.tools = [new Gef.jbs.tool.TaskTool(), new Gef.jbs.tool.EndTool(), new Gef.jbs.tool.GatewayTool(), new Gef.jbs.tool.LineTool(), 
...
Gef.jbs.xml.JBSDeserializer = Gef.extend(Gef.gef.xml.XmlDeserializer, {
    ...
    parseRoot: function(_) {
        ...
        _.decode($, ["auto", "end-cancel", "counter-sign", "custom", "decision", "end", "end-error", "fork", "group", "hql", "human", "java", "jms", "join", "mail", "rule-decision", "rules", "script", "sql", "start", "state", "sub-process", "task", "foreach"]);
        Gef.each($.childNodes,
        function($) {
            this.parseNodes($, _)
        },
        this);
        Gef.each(_.getChildren(),
        function($) {
...
Raphael = (function() {
    var j = /[, ]+/,
    C0 = /^(circle|rect|path|ellipse|text|image)$/,
    O = "prototype",
    Q = "hasOwnProperty",
    T = document,
    U0 = window,
    ...
    k0 = function() {
        if (k0.is(arguments[0], "array")) {
            var _ = arguments[0],
            A = z[c](k0, _.splice(0, 3 + k0.is(_[0], g0))),
            B = A.set();
            for (var C = 0,
            D = _[k]; C < D; C++) {
                var $ = _[C] || {};
    ...
    e0 = " ",
    E = "split",
    L = "click dblclick mousedown mousemove mouseout mouseover mouseup" [E](e0),
    a0 = "join",
    k = "length",
    A0 = String[O].toLowerCase,
    $1 = Math,
    q = $1.max,
    D0 = $1.min,
    g0 = "number",
    ...
    Z = /^(?=[\da-f]$)/,
    i = /^url\(['"]?([^\)]+?)['"]?\)$/i,
    B = /^\s*((#[a-f\d]{6})|(#[a-f\d]{3})|rgb\(\s*([\d\.]+\s*,\s*[\d\.]+\s*,\s*[\d\.]+)\s*\)|rgb\(\s*([\d\.]+%\s*,\s*[\d\.]+%\s*,\s*[\d\.]+%)\s*\)|hs[bl]\(\s*([\d\.]+\s*,\s*[\d\.]+\s*,\s*[\d\.]+)\s*\)|hs[bl]\(\s*([\d\.]+%\s*,\s*[\d\.]+%\s*,\s*[\d\.]+%)\s*\))\s*$/i,
    N = $1.round,
    $0 = "setAttribute",
    v0 = parseFloat,
    M = parseInt,
    J0 = String[O].toUpperCase,
    p = {
        blur: 0,
    ...
    B0 = "replace";
    k0.version = "1.3.2";
    k0.type = (U0.SVGAngle || T.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#BasicStructure", "1.1") ? "SVG": "VML");
    if (k0.type == "VML") {
        var l0 = T.createElement("div");
        l0.innerHTML = "<!--[if vml]><br><br><![endif]-->";
        if (l0.childNodes[k] != 2) return k0.type = null;
        l0 = null
    }
    ...
    k0.is = function(_, $) {
        $ = A0.call($);
        return (($ == "object" || $ == "undefined") && typeof _ == $) || (_ == null && $ == "null") || A0.call(X0.call(_).slice(8, -1)) == $
    };
    ...
    var P0 = function(A) {
        if (k0.vml) {
            ...
            P0 = s0(function(_) {
                $.style.color = _;
                return T.defaultView.getComputedStyle($, T0).getPropertyValue("color")
            })
        }
        return P0(A)
    },
    ...
    function s0($, A, _) {
        function B() {
            var D = Array[O].slice.call(arguments, 0),
            E = D[a0]("?"),
            C = B.cache = B.cache || {},
            F = B.count = B.count || [];
            if (C[Q](E)) return _ ? _(C[E]) : C[E];
    ...
    var V0 = /([achlmqstvz])[\s,]*((-?\d*\.?\d*(?:e[-+]?\d+)?\s*,?\s*)+)/ig,
    f0 = /(-?\d*\.?\d*(?:e[-+]?\d+)?)\s*,?\s*/ig;
    k0.parsePathString = s0(function($) {
        ...
        _ = [];
        if (k0.is($, "array") && k0.is($[0], "array")) _ = W0($);
        if (!_[k])($ + T0)[B0](V0,
        function(C, $, E) {
            var D = [],
            B = A0.call($);
    ...
    k0.findDotsAtSegment = function(A, _, F, D, Q, L, S, R, H) {
        var P = 1 - H,
        O = H0(P, 3) * A + H0(P, 2) * 3 * H * F + P * 3 * H * H * Q + H0(H, 3) * S,
        M = H0(P, 3) * _ + H0(P, 2) * 3 * H * D + P * 3 * H * H * L + H0(H, 3) * R,
        K = A + 2 * H * (F - A) + H * H * (Q - 2 * F + A),
        J = _ + 2 * H * (D - _) + H * H * (L - 2 * D + _),
        E = F + 2 * H * (Q - F) + H * H * (S - 2 * Q + F),
        C = D + 2 * H * (L - D) + H * H * (R - 2 * L + D),
        G = (1 - H) * A + H * F,
        N = (1 - H) * _ + H * D,
        $ = (1 - H) * Q + H * S,
        B = (1 - H) * L + H * R,
        I = (90 - $1.atan((K - E) / (J - C)) * 180 / $1.PI); (K > E || J < C) && (I += 180);
        return {
            x: O,
            y: M,
            m: {
    ...
    W0 = function(D) {
        var $ = [];
        if (!k0.is(D, "array") || !k0.is(D && D[0], "array")) D = k0.parsePathString(D);
        for (var A = 0,
        C = D[k]; A < C; A++) {
            $[A] = [];
            for (var _ = 0,
            B = D[A][k]; _ < B; _++) $[A][_] = D[A][_]
        }
        $[M0] = k0._path2string;
        return $
    },
    y0 = s0(function(F) {
        if (!k0.is(F, "array") || !k0.is(F && F[0], "array")) F = k0.parsePathString(F);
        var I = [],
        K = 0,
        J = 0,
        E = 0,
        ...
        G = F[k]; M < G; M++) {
            ...
            if (B[0] != A0.call(B[0])) {
                ...
                default:
                    for (var L = 1,
                    N = B[k]; L < N; L++) _[L] = +(B[L] - ((L % 2) ? K: J)).toFixed(3)
                }
            } else {
                _ = I[M] = [];
                if (B[0] == "m") {
    ...
    u = s0(function(F) {
        ...
        E = F[k]; L < E; L++) {
            ...
            if (J[0] != J0.call(J[0])) {
                ...
                default:
                    for (var K = 1,
                    M = J[k]; K < M; K++) _[K] = +J[K] + ((K % 2) ? I: H)
                }
            } else for (var D = 0,
            A = J[k]; D < A; D++) G[L][D] = J[D];
            switch (_[0]) {
            case "Z":
                I = C;
                H = B;
    ...
    F0 = function(A, $, D, C, _, F) {
        var B = 1 / 3,
        E = 2 / 3;
        return [B * A + E * D, B * $ + E * C, B * _ + E * D, B * F + E * C, _, F]
    },
    I = function(S, l, J, G, U, X, R, Q, j, L) {
        ...
        if (!L) {
            ...
            var $ = J * J,
            o = G * G,
            u = (X == R ? -1 : 1) * $1.sqrt($1.abs(($ * o - $ * _ * _ - o * A * A) / ($ * _ * _ + o * A * A))),
            N = u * J * _ / G + (S + Q) / 2,
            M = u * -G * A / J + (l + j) / 2,
            W = $1.asin(((l - M) / G).toFixed(7)),
            c = $1.asin(((j - M) / G).toFixed(7));
            W = S < N ? T - W: W;
            c = Q < N ? T - c: c;
            W < 0 && (W = T * 2 + W);
            c < 0 && (c = T * 2 + c);
        ...
        h[0] = 2 * f[0] - h[0];
        h[1] = 2 * f[1] - h[1];
        if (L) return [h, t, v][V](D);
        else {
            D = [h, t, v][V](D)[a0]()[E](",");
            var B = [];
            for (var s = 0,
            F = D[k]; s < F; s++) B[s] = s % 2 ? g(D[s - 1], D[s], d).y: g(D[s], D[s + 1], d).x;
            return B
        }
    },
    P = function(A, _, $, H, G, F, E, B, D) {
        var C = 1 - D;
        return {
            x: H0(C, 3) * A + H0(C, 2) * 3 * D * $ + C * 3 * D * D * G + H0(D, 3) * E,
            y: H0(C, 3) * _ + H0(C, 2) * 3 * D * H + C * 3 * D * D * F + H0(D, 3) * B
        }
    },
    O0 = s0(function(B, _, G, $, F, D, J, N) {
        var C = (F - 2 * G + B) - (J - 2 * F + G),
        I = 2 * (G - B) - 2 * (F - G),
        M = B - G,
    ...
    F = s0(function(E, H) {
        ...
        L = function(A, $) {
            var B, _;
            if (!A) return ["C", $.x, $.y, $.x, $.y, $.x, $.y]; ! (A[0] in {
                T: 1,
                Q: 1
            }) && ($.qx = $.qy = null);
            switch (A[0]) {
        ...
        A = function(A, $) {
            if (A[$][k] > 7) {
                A[$].shift();
                var _ = A[$];
                while (_[k]) A.splice($++, 0, ["C"][V](_.splice(0, 6)));
                A.splice($, 1);
                B = q(F[k], J && J[k] || 0)
            }
        },
    ...
    w = s0(function(D) {
        ...
        E = D[k]; F < E; F++) {
            ...
            if (_.color.error) return null;
            _.color = _.color.hex;
            H[2] && (_.offset = H[2] + "%");
            C[f](_)
        }
        for (F = 1, E = C[k] - 1; F < E; F++) if (!C[F].offset) {
            var $ = v0(C[F - 1].offset || 0),
            B = 0;
            for (var A = F + 1; A < E; A++) if (C[A].offset) {
                B = C[A].offset;
                break
            }
            if (!B) {
    ...
    j0 = function(_, B, C, $) {
        ...
        } else if (k0.is(_, g0) && $ != null) return {
            container: 1,
            x: _,
            y: B,
            width: C,
    ...
    S0 = function(_, B) {
        var A = this;
        for (var $ in B) if (B[Q]($) && !($ in _)) switch (typeof B[$]) {
        case "function":
            (function(B) {
                _[$] = _ === A ? B: function() {
    ...
    t = function($) {
        return function() {
            throw new Error("Rapha\xebl: you are calling to method “" + $ + "” of removed object")
        }
    },
    b0 = /^r(?:\(([^,]+?)\s*,\s*([^\)]+?)\))?/;
    if (k0.svg) {
        ...
        var e = function(B) {
            for (var A = 0,
            $ = B[k]; A < $; A++) if (A0.call(B[A][0]) != "a") {
                for (var _ = 1,
                C = B[A][k]; _ < C; _++) B[A][_] = N(B[A][_])
            } else {
                B[A][6] = N(B[A][6]);
                B[A][7] = N(B[A][7])
            }
        ...
        G0 = function(A, $) {
            if ($) {
                for (var _ in $) if ($[Q](_)) A[$0](_, $[_] + T0)
            } else return T.createElementNS(X[O].svgns, A)
        };
        k0[M0] = function() {
        ...
        h = function(G, C, _) {
            ...
            function(_, B, A) {
                ...
                if (B && A) {
                    ...
                    M = v0(A);
                    var $ = ((M > 0.5) * 2 - 1);
                    H0(O - 0.5, 2) + H0(M - 0.5, 2) > 0.25 && (M = $1.sqrt(0.25 - H0(O - 0.5, 2)) * $ + 0.5) && M != 0.5 && (M = M.toFixed(5) - 0.00001 * $)
                }
                return T0
            });
            C = C[E](/\s*\-\s*/);
            if (K == "linear") {
                ...
                var F = [0, 0, $1.cos(N * $1.PI / 180), $1.sin(N * $1.PI / 180)],
                B = 1 / (q($1.abs(F[2]), $1.abs(F[3])) || 1);
                F[2] *= B;
                F[3] *= B;
                if (F[2] < 0) {
                    F[0] = -F[2];
            ...
            D = J[k]; I < D; I++) {
                var L = G0("stop");
                G0(L, {
                    offset: J[I].offset ? J[I].offset: !I ? "0%": "100%",
        ...
        S = function(_) {
            var $ = _.getBBox();
            G0(_.pattern, {
                patternTransform: k0.format("translate({0},{1})", $.x, $.y)
            })
        },
        z0 = function(Y, C) {
            var O = {
            ...
            P = function(A, B) {
                B = O[A0.call(B)];
                if (B) {
                    var _ = A.attrs["stroke-width"] || "1",
                    E = {
                        round: _,
                        square: _,
                        butt: 0
                    } [A.attrs["stroke-linecap"] || C["stroke-linecap"]] || 0,
                    $ = [],
                    D = B[k];
                    while (D--) $[D] = B[D] * _ + ((D % 2) ? 1 : -1) * E;
                    G0(F, {
                        "stroke-dasharray": $[a0](",")
                    })
                }
            ...
            for (var U in C) if (C[Q](U)) {
                ...
                case "clip-rect":
                    var A = (L + T0)[E](j);
                    if (A[k] == 4) {
                        Y.clip && Y.clip.parentNode.parentNode.removeChild(Y.clip.parentNode);
                        var D = G0("clipPath"),
                        G = G0("rect");
                        D.id = "r" + (k0._id++)[M0](36);
                        G0(G, {
                    ...
                    if (!L) {
                        var I = T.getElementById(F.getAttribute("clip-path")[B0](/(^url\(#|\)$)/g, T0));
                ...
                case "x":
                    if (J.fx) L = -J.x - (J.width || 0);
                case "rx":
                    if (U == "rx" && Y.type == "rect") break;
                case "cx":
                ...
                case "y":
                    if (J.fy) L = -J.y - (J.height || 0);
                case "ry":
                    if (U == "ry" && Y.type == "rect") break;
                case "cy":
                ...
                case "r":
                    ...
                    break;
                case "src":
                    if (Y.type == "image") F.setAttributeNS(Y.paper.xlink, "href", L);
                    break;
                case "stroke-width":
                    F.style.strokeWidth = L;
                    F[$0](U, L);
                    if (J["stroke-dasharray"]) P(Y, J["stroke-dasharray"]);
                    break;
                ...
                case "scale":
                    Z = (L + T0)[E](j);
                    Y.scale( + Z[0] || 1, +Z[1] || +Z[0] || 1, isNaN(v0(Z[2])) ? null: +Z[2], isNaN(v0(Z[3])) ? null: +Z[3]);
                    break;
                case "fill":
                    var R = (L + T0).match(i);
                    if (R) {
                    ...
                    if (!k0.getRGB(L).error) {
                        delete C.gradient;
                        delete J.gradient; ! k0.is(J.opacity, "undefined") && k0.is(C.opacity, "undefined") && G0(F, {
                            opacity: J.opacity
                        }); ! k0.is(J["fill-opacity"], "undefined") && k0.is(C["fill-opacity"], "undefined") && G0(F, {
                            "fill-opacity": J["fill-opacity"]
                        })
                    } else if ((({
                        circle: 1,
                        ellipse: 1
                    })[Q](Y.type) || (L + T0).charAt() != "r") && h(F, L, Y.paper)) {
                        J.gradient = L;
                        J.fill = "none";
                        break
                    }
                ...
                case "fill-opacity":
                    if (J.gradient) {
                        var _ = T.getElementById(F.getAttribute("fill")[B0](/^url\(#|\)$/g, T0));
                        if (_) {
            ...
            K(Y, C);
            if (b) Y.rotate(b.join(e0));
            else v0(V) && Y.rotate(V, true)
        },
        o = 1.2,
        K = function(_, C) {
            if (_.type != "text" || !(C[Q]("text") || C[Q]("font") || C[Q]("font-size") || C[Q]("x") || C[Q]("y"))) return;
            var D = _.attrs,
            A = _.node,
            G = A.firstChild ? M(T.defaultView.getComputedStyle(A.firstChild, T0).getPropertyValue("font-size"), 10) : 10;
            if (C[Q]("text")) {
                D.text = C.text;
        ...
        Y0[O].rotate = function(A, _, $) {
            ...
            if (A == null) {
                if (this._.rt.cx) return [this._.rt.deg, this._.rt.cx, this._.rt.cy][a0](e0);
            ...
            A = v0(A[0]);
            if (_ != null) this._.rt.deg = A;
            else this._.rt.deg += A; ($ == null) && (_ = null);
            this._.rt.cx = _;
            this._.rt.cy = $;
            _ = _ == null ? B.x + B.width / 2 : _;
            $ = $ == null ? B.y + B.height / 2 : $;
            if (this._.rt.deg) {
                this.transformations[0] = k0.format("rotate({0} {1} {2})", this._.rt.deg, _, $);
                this.clip && G0(this.clip, {
                    transform: k0.format("rotate({0} {1} {2})", -this._.rt.deg, _, $)
                })
            } else {
        ...
        Y0[O].hide = function() { ! this.removed && (this.node.style.display = "none");
            return this
        };
        Y0[O].show = function() { ! this.removed && (this.node.style.display = "");
            return this
        };
        Y0[O].remove = function() {
            ...
            for (var $ in this) delete this[$];
            this.removed = true
        };
        Y0[O].getBBox = function() {
            if (this.removed) return this;
            if (this.type == "path") return Y(this.attrs.path);
            ...
            if (this.type == "text") {
                ...
                for (var _ = 0,
                B = this.node.getNumberOfChars(); _ < B; _++) {
                    var C = this.node.getExtentOfChar(_); (C.y < D.y) && (D.y = C.y); (C.y + C.height - D.y > D.height) && (D.height = C.y + C.height - D.y); (C.x + C.width - D.x > D.width) && (D.width = C.x + C.width - D.x)
                }
            }
            $ && this.hide();
            return D
        ...
        Y0[O].attr = function($, E) {
            ...
            if ($ == null) {
                var B = {};
                for (var C in this.attrs) if (this.attrs[Q](C)) B[C] = this.attrs[C];
                this._.rt.deg && (B.rotation = this.rotate()); (this._.sx != 1 || this._.sy != 1) && (B.scale = this.scale());
                B.gradient && B.fill == "none" && (B.fill = B.gradient) && delete B.gradient;
                return B
            }
            if (E == null && k0.is($, "string")) {
                if ($ == "translation") return s.call(this);
                if ($ == "rotation") return this.rotate();
                if ($ == "scale") return this.scale();
                if ($ == "fill" && this.attrs.fill == "none" && this.attrs.gradient) return this.attrs.gradient;
                return this.attrs[$]
            }
            if (E == null && k0.is($, "array")) {
                var _ = {};
                for (var A = 0,
                D = $.length; A < D; A++) _[$[A]] = this.attr($[A]);
                return _
            }
            if (E != null) {
                var F = {};
                F[$] = E;
                z0(this, F)
            } else if ($ != null && k0.is($, "object")) z0(this, $);
            return this
        };
        ...
        Y0[O].insertAfter = function(_) {
            if (this.removed) return this;
            var A = _.node;
            if (A.nextSibling) A.parentNode.insertBefore(this.node, A.nextSibling);
            else A.parentNode[I0](this.node);
            $(this, _, this.paper);
            return this
        };
        ...
        z = function() {
            ...
            if (E == 1) {
                B.style.cssText = "position:absolute;left:" + A + "px;top:" + D + "px";
                T.body[I0](B)
            } else if (E.firstChild) E.insertBefore(B, E.firstChild);
            else E[I0](B);
            E = new X;
            E.width = C;
            E.height = _;
        ...
        X[O].clear = function() {
            ...
            this.bottom = this.top = null; (this.desc = G0("desc"))[I0](T.createTextNode("Created with Rapha\xebl"));
            $[I0](this.desc);
            $[I0](this.defs = G0("defs"))
        };
        X[O].remove = function() {
            this.canvas.parentNode && this.canvas.parentNode.removeChild(this.canvas);
            for (var $ in this) this[$] = t($)
        }
    }
    if (k0.vml) {
        var J = {
        ...
        E0 = function(G) {
            ...
            E = D[k]; B < E; B++) {
                ...
                _ == "z" && (_ = "x");
                for (var C = 1,
                H = A[k]; C < H; C++) _ += N(A[C] * y) + (C != H - 1 ? ",": T0);
                I[f](_)
            }
            return I[a0](e0)
        };
        ...
        v = function(E, A) {
            var B = m0("group");
            B.style.cssText = "position:absolute;left:0;top:0;width:" + A.width + "px;height:" + A.height + "px";
        ...
        z0 = function(M, H) {
            ...
            S = R.style,
            $, F = M;
            for (var U in H) if (H[Q](U)) I[U] = H[U];
            ...
            if (H.rotation != null) M.rotate(H.rotation, true);
            if (H.translation) {
            ...
            if (H.scale) {
                $ = (H.scale + T0)[E](j);
                M.scale( + $[0] || 1, +$[1] || +$[0] || 1, +$[2] || null, +$[3] || null)
            }
            if ("clip-rect" in H) {
                ...
                if (_[k] == 4) {
                    _[2] = +_[2] + ( + _[0]);
                    _[3] = +_[3] + ( + _[1]);
                    var V = R.clipRect || T.createElement("div"),
                    D = V.style,
                    J = R.parentNode;
                ...
                if (!H["clip-rect"]) R.clipRect && (R.clipRect.style.clip = T0)
            }
            if (M.type == "image" && H.src) R.src = H.src;
            if (M.type == "image" && H.opacity) {
                R.filterOpacity = " progid:DXImageTransform.Microsoft.Alpha(opacity=" + (H.opacity * 100) + ")";
                S.filter = (R.filterMatrix || T0) + (R.filterOpacity || T0)
            }
            H.font && (S.font = H.font);
            H["font-family"] && (S.fontFamily = "\"" + H["font-family"][E](",")[0][B0](/^['"]+|['"]+$/g, T0) + "\"");
            H["font-size"] && (S.fontSize = H["font-size"]);
            H["font-weight"] && (S.fontWeight = H["font-weight"]);
            H["font-style"] && (S.fontStyle = H["font-style"]);
            if (H.opacity != null || H["stroke-width"] != null || H.fill != null || H.stroke != null || H["stroke-width"] != null || H["stroke-opacity"] != null || H["fill-opacity"] != null || H["stroke-dasharray"] != null || H["stroke-miterlimit"] != null || H["stroke-linejoin"] != null || H["stroke-linecap"] != null) {
                R = M.shape || R;
                var G = (R.getElementsByTagName("fill") && R.getElementsByTagName("fill")[0]),
                L = false; ! G && (L = G = m0("fill"));
                if ("fill-opacity" in H || "opacity" in H) {
                    var A = (( + I["fill-opacity"] + 1 || 2) - 1) * (( + I.opacity + 1 || 2) - 1);
                    A < 0 && (A = 0);
                    A > 1 && (A = 1);
                    G.opacity = A
                }
                H.fill && (G.on = true);
                if (G.on == null || H.fill == "none") G.on = false;
                if (G.on && H.fill) {
                ...
                L && R[I0](G);
                var K = (R.getElementsByTagName("stroke") && R.getElementsByTagName("stroke")[0]),
                C = false; ! K && (C = K = m0("stroke"));
                if ((H.stroke && H.stroke != "none") || H["stroke-width"] || H["stroke-opacity"] != null || H["stroke-dasharray"] || H["stroke-miterlimit"] || H["stroke-linejoin"] || H["stroke-linecap"]) K.on = true; (H.stroke == "none" || K.on == null || H.stroke == 0 || H["stroke-width"] == 0) && (K.on = false);
                K.on && H.stroke && (K.color = k0.getRGB(H.stroke).hex);
                A = (( + I["stroke-opacity"] + 1 || 2) - 1) * (( + I.opacity + 1 || 2) - 1);
                ...
                K.opacity = A;
                H["stroke-linejoin"] && (K.joinstyle = H["stroke-linejoin"] || "miter");
                K.miterlimit = H["stroke-miterlimit"] || 8;
                H["stroke-linecap"] && (K.endcap = H["stroke-linecap"] == "butt" ? "flat": H["stroke-linecap"] == "square" ? "square": "round");
                if (H["stroke-dasharray"]) {
                    ...
                    K.dashstyle = P[Q](H["stroke-dasharray"]) ? P[H["stroke-dasharray"]] : T0
                }
                C && R[I0](K)
            }
            if (F.type == "text") {
                ...
                F.node.string && (F.paper.span.innerHTML = (F.node.string + T0)[B0](/</g, "&#60;")[B0](/&/g, "&#38;")[B0](/\n/g, "<br>"));
        ...
        h = function(_, I) {
            _.attrs = _.attrs || {};
            var J = _.attrs,
            F = _.node.getElementsByTagName("fill"),
            B = "linear",
            H = ".5 .5";
            _.attrs.gradient = I;
            I = (I + T0)[B0](b0,
            function(_, $, A) {
                ...
                if ($ && A) {
                    $ = v0($);
                    A = v0(A);
                    H0($ - 0.5, 2) + H0(A - 0.5, 2) > 0.25 && (A = $1.sqrt(0.25 - H0($ - 0.5, 2)) * ((A > 0.5) * 2 - 1) + 0.5);
                    H = $ + e0 + A
                }
                return T0
            });
            ...
            if (C[k]) {
                ...
                var G = [];
                for (var $ = 0,
                D = C[k]; $ < D; $++) C[$].offset && G[f](C[$].offset + e0 + C[$].color);
                F.colors && (F.colors.value = G[k] ? G[a0](",") : "0% " + F.color);
                if (B == "radial") {
                    F.focus = "100%";
                    F.focussize = H;
                    F.focusposition = H
                } else F.angle = (270 - A) % 360
            }
            return 1
        };
        Y0 = function(C, D, _) {
        ...
        Y0[O].rotate = function(_, $, A) {
            ...
            _ = v0(_[0]);
            if ($ != null) this._.rt.deg = _;
            else this._.rt.deg += _;
            A == null && ($ = null);
            this._.rt.cx = $;
            this._.rt.cy = A;
        ...
        Y0[O].setBox = function(C, A, _) {
            ...
            C = C || {};
            for (var J in C) if (C[Q](J)) this.attrs[J] = C[J];
            A = A || this._.rt.cx;
            _ = _ || this._.rt.cy;
            var F = this.attrs,
            R, P, S, B;
            ...
            case "image":
                ...
                break;
            case "text":
                this.textpath.v = ["m", N(F.x), ", ", N(F.y - 2), "l", N(F.x) + 1, ", ", N(F.y - 2)][a0](T0);
                R = F.x - N(this.W / 2);
                P = F.y - this.H / 2;
                S = this.W;
                B = this.H;
            ...
            A = (A == null) ? R + S / 2 : A;
            _ = (_ == null) ? P + B / 2 : _;
            var $ = A - this.paper.width / 2,
            M = _ - this.paper.height / 2,
            L;
            ...
            if (this.type == "path") {
                ...
                var G = ( + C.r || 0) / D0(S, B);
                if (this.type == "rect" && this.arcsize.toFixed(4) != G.toFixed(4) && (G || this.arcsize)) {
                    var E = m0("roundrect"),
                    D = {},
                    K = this.events && this.events[k];
                    J = 0;
                    E.arcsize = G;
                    E.raphael = this;
                    this.Group[I0](E);
                    ...
                    delete D.scale;
                    this.attr(D);
                    if (this.events) for (; J < K; J++) this.events[J].unbind = n0(this.node, this.events[J].name, this.events[J].f, this)
                }
            }
        };
        Y0[O].hide = function() { ! this.removed && (this.Group.style.display = "none");
            return this
        };
        Y0[O].show = function() { ! this.removed && (this.Group.style.display = "block");
            return this
        };
        Y0[O].getBBox = function() {
            if (this.removed) return this;
        ...
        Y0[O].attr = function(A, D) {
            ...
            if (A == null) {
                var C = {};
                for (var $ in this.attrs) if (this.attrs[Q]($)) C[$] = this.attrs[$];
                this._.rt.deg && (C.rotation = this.rotate()); (this._.sx != 1 || this._.sy != 1) && (C.scale = this.scale());
                C.gradient && C.fill == "none" && (C.fill = C.gradient) && delete C.gradient;
                return C
            }
            if (D == null && k0.is(A, "string")) {
                if (A == "translation") return s.call(this);
            ...
            if (E) {
                if (E.text && this.type == "text") this.node.string = E.text;
                z0(this, E);
                if (E.gradient && (({
                    circle: 1,
                    ellipse: 1
                })[Q](this.type) || (E.gradient + T0).charAt() != "r")) h(this, E.gradient); (this.type != "path" || this._.rt.deg) && this.setBox(this.attrs)
            }
            return this
        };
        Y0[O].toFront = function() { ! this.removed && this.Group.parentNode[I0](this.Group);
        ...
        Y0[O].insertAfter = function(_) {
            if (this.removed) return this;
            if (_.Group.nextSibling) _.Group.parentNode.insertBefore(this.Group, _.Group.nextSibling);
            else _.Group.parentNode[I0](this.Group);
            $(this, _, this.paper);
            return this
        };
        Y0[O].insertBefore = function($) {
        ...
        Y0[O].blur = function($) {
            var _ = this.node.style,
            A = _.filter;
            A = A.replace(W, "");
            if ( + $ !== 0) {
                this.attrs.blur = $;
                _.filter = A + " progid:DXImageTransform.Microsoft.Blur(pixelradius=" + ( + $ || 1.5) + ")";
                _.margin = Raphael.format("-{0}px 0 0 -{0}px", Math.round( + $ || 1.5))
            } else {
                _.filter = A;
                _.margin = 0;
        ...
        R = function(A, _, E, B) {
            ...
            D = m0("oval"),
            F = D.style;
            C.style.cssText = "position:absolute;left:0;top:0;width:" + A.width + "px;height:" + A.height + "px";
            C.coordsize = L0;
            C.coordorigin = A.coordorigin;
            C[I0](D);
            var $ = new Y0(D, C, A);
        ...
        r0 = function(_, G, E, F, A) {
            ...
            $ = m0("oval"),
            D = $.style;
            C.style.cssText = "position:absolute;left:0;top:0;width:" + _.width + "px;height:" + _.height + "px";
            C.coordsize = L0;
        ...
        w0 = function(A, I, G, D) {
            ...
            C.coordsize = L0;
            C.coordorigin = A.coordorigin;
            E.v = k0.format("m{0},{1}l{2},{1}", N(I * 10), N(G * 10), N(I * 10) + 1);
            E.textpathok = true;
            F.width = A.width;
            F.height = A.height;
            H.string = D + T0;
        ...
        try { ! T.namespaces.rvml && T.namespaces.add("rvml", "urn:schemas-microsoft-com:vml");
            m0 = function($) {
                return T.createElement("<rvml:" + $ + " class=\"rvml\">")
            }
        } catch(o0) {
            m0 = function($) {
                return T.createElement("<" + $ + " xmlns=\"urn:schemas-microsoft.com:vml\" class=\"rvml\">")
            }
        }
        z = function() {
            var H = j0[c](0, arguments),
            _ = H.container,
            G = H.height,
            D,
            A = H.width,
            F = H.x,
            E = H.y;
            if (!_) throw new Error("VML container not found.");
            var C = new X,
            B = C.canvas = T.createElement("div"),
            $ = B.style;
            A = A || 512;
            G = G || 342;
            A == +A && (A += "px");
            ...
            B[I0](C.span);
            $.cssText = k0.format("width:{0};height:{1};position:absolute;clip:rect(0 {0} {1} 0);overflow:hidden", A, G);
            if (_ == 1) {
                T.body[I0](B);
                $.left = F + "px";
                $.top = E + "px"
                ...
                _.style.width = A;
                _.style.height = G;
                if (_.firstChild) _.insertBefore(B, _.firstChild);
                else _[I0](B)
            }
            S0.call(C, C, k0.fn);
            return C
    ...
    if ((/^Apple|^Google/).test(U0.navigator.vendor) && !(U0.navigator.userAgent.indexOf("Version/4.0") + 1)) X[O].safari = function() {
        var $ = this.rect( - 99, -99, this.width + 99, this.height + 99);
        U0.setTimeout(function() {
            $.remove()
        })
    };
    else X[O].safari = function() {};
    var n0 = (function() {
        if (T.addEventListener) return function(B, C, A, _) {
            var $ = function($) {
                return A.call(_, $)
            };
        ...
        else if (T.attachEvent) return function(B, $, D, A) {
            var C = function($) {
    ...
    for (var x0 = L[k]; x0--;)(function($) {
        ...
        Y0[O]["un" + $] = function(_) {
            var B = this.events,
            A = B[k];
            while (A--) if (B[A].name == $ && B[A].f == _) {
                B[A].unbind();
                B.splice(A, 1); ! B.length && delete this.events;
                return this
            }
            return this
        }
    ...
    X[O].set = function($) {
        arguments[k] > 1 && ($ = Array[O].splice.call(arguments, 0, arguments[k]));
        return new a($)
    };
    X[O].setSize = b;
        ...
        if (X != 0) {
            ...
            case "path":
                ...
                J = G[k]; H < J; H++) {
                    ...
                    if (P == "A") {
                        ...
                        for (I = 1, C = M[k]; I < C; I++) M[I] *= B
                    } else for (I = 1, C = M[k]; I < C; I++) M[I] *= (I % 2) ? E: B
                }
                var c = Y(G);
                S = A - c.x - c.width / 2;
                O = _ - c.y - c.height / 2;
            ...
            if (this.type in {
                ...
                if (this.transformations) {
                    ...
                    this.node.filterMatrix = " progid:DXImageTransform.Microsoft.Matrix(M11=" [V](T, ", M12=0, M21=0, M22=", Z, ", Dx=0, Dy=0, sizingmethod='auto expand', filtertype='bilinear')");
                    L.filter = (this.node.filterMatrix || T0) + (this.node.filterOpacity || T0)
                }
        ...
        return function(D, E, L) {
            ...
            B = D.length; N < B; N++) {
                ...
                if (A[0] == "M") {
                    ...
                    if (H + M > E) {
                        if (_ && !C.start) {
                            K = g(J, I, A[1], A[2], A[3], A[4], A[5], A[6], E - H);
                            G += ["C", K.start.x, K.start.y, K.m.x, K.m.y, K.x, K.y];
                            if (L) return G;
                            C.start = G;
                            G = ["M", K.x, K.y + "C", K.n.x, K.n.y, K.end.x, K.end.y, A[5], A[6]][a0]();
                            H += M;
                            J = +A[5];
                            I = +A[6];
                            continue
            ...
            C.end = G;
            K = $ ? H: _ ? C: k0.findDotsAtSegment(J, I, A[1], A[2], A[3], A[4], A[5], A[6], 1);
            K.alpha && (K = {
                x: K.x,
                y: K.y,
                alpha: K.alpha
    ...
    Y0[O].getSubpath = function(A, _) {
        if (this.type != "path") return;
        if ($1.abs(this.getTotalLength() - _) < 0.000001) return H(this.attrs.path, A).end;
    ...
    U = function() {
        var a = +new Date;
        for (var K in G) if (K != "length" && G[Q](K)) {
            var B = G[K];
            ...
            b;
            if (Y < H) {
                var P = k0.easing_formulas[R] ? k0.easing_formulas[R](Y / H) : Y / H;
                for (var I in D) if (D[Q](I)) {
                    switch (u0[I]) {
                    case "along":
                        b = P * H * L[I];
                        $.back && (b = $.len - b);
                        var J = A($[I], b);
                        Z.translate(L.sx - L.x || 0, L.sy - L.y || 0);
                        L.x = J.x;
                        L.y = J.y;
                    ...
                    case "number":
                        ...
                        break;
                    case "colour":
                        b = "rgb(" + [_(N(D[I].r + P * H * L[I].r)), _(N(D[I].g + P * H * L[I].g)), _(N(D[I].b + P * H * L[I].b))][a0](",") + ")";
                        break;
                    case "path":
                        b = [];
                        for (var F = 0,
                        W = D[I][k]; F < W; F++) {
                            b[F] = [D[I][F][0]];
                            for (var C = 1,
                            E = D[I][F][k]; C < E; C++) b[F][C] = +D[I][F][C] + P * H * L[I][F][C];
                            b[F] = b[F][a0](e0)
                        }
                        b = b[a0](e0);
                        break;
                    case "csv":
                        ...
                        case "rotation":
                            ...
                            break;
                        case "scale":
                            b = [ + D[I][0] + P * H * L[I][0], +D[I][1] + P * H * L[I][1], (2 in $[I] ? $[I][2] : T0), (3 in $[I] ? $[I][3] : T0)][a0](e0);
                            break;
                        case "clip-rect":
                            b = [];
                            F = 4;
                            while (F--) b[F] = +D[I][F] + P * H * L[I][F];
                            break
                        }
                        break
                    }
                ...
                if ($.along) {
                    J = A($.along, $.len * !$.back);
                    Z.translate(L.sx - (L.x || 0) + J.x - L.sx, L.sy - (L.y || 0) + J.y - L.sy);
                    $.rot && Z.rotate(L.r + J.alpha, J.x, J.y)
    ...
    Y0[O].animate = function(I, S, R, $) {
        if (k0.is(R, "function") || !R) $ = R || null;
        var M = {},
        B = {},
        X = {};
        for (var T in I) if (I[Q](T)) if (u0[Q](T)) {
            M[T] = this.attr(T); (M[T] == null) && (M[T] = p[T]);
            B[T] = I[T];
            switch (u0[T]) {
            case "along":
                var D = i0(I[T]),
            ...
            case "path":
                ...
                W = M[T][k]; C < W; C++) {
                    X[T][C] = [0];
                    for (var N = 1,
                    H = M[T][C][k]; N < H; N++) X[T][C][N] = (P[C][N] - M[T][C][N]) / S
                }
                break;
            case "csv":
                var _ = (I[T] + T0)[E](j),
                ...
                case "translation":
                    ...
                    break;
                case "rotation":
                    M[T] = (V[1] == _[1] && V[2] == _[2]) ? V: [0, _[1], _[2]];
                    X[T] = [(_[0] - M[T][0]) / S, 0, 0];
                    break;
                case "scale":
                    I[T] = _;
                    M[T] = (M[T] + T0)[E](j);
                    X[T] = [(_[0] - M[T][0]) / S, (_[1] - M[T][1]) / S, 0, 0];
                    break;
                case "clip-rect":
                    M[T] = (M[T] + T0)[E](j);
                    X[T] = [];
                    C = 4;
                    while (C--) X[T][C] = (_[C] - M[T][C]) / S;
                    break
                }
                B[T] = _
            }
    ...
    var a = function(_) {
        ...
        this[k] = 0;
        if (_) for (var A = 0,
        $ = _[k]; A < $; A++) if (_[A] && (_[A].constructor == Y0 || _[A].constructor == a)) {
    ...
    for (var C in Y0[O]) if (Y0[O][Q](C)) a[O][C] = (function($) {
        return function() {
            for (var A = 0,
            _ = this.items[k]; A < _; A++) this.items[A][$][c](this.items[A], arguments);
            return this
        }
    })(C);
    a[O].attr = function(A, D) {
        if (A && k0.is(A, "array") && k0.is(A[0], "object")) {
            for (var _ = 0,
            B = A[k]; _ < B; _++) this.items[_].attr(A[_])
        } else for (var $ = 0,
        C = this.items[k]; $ < C; $++) this.items[$].attr(A, D);
        return this
    };
    a[O].animate = function(B, A, F, E) { (k0.is(F, "function") || !F) && (E = F || null);
        var _ = this.items[k],
        $ = _,
        D = this,
        C;
        E && (C = function() { ! --_ && E.call(D)
        });
        this.items[--$].animate(B, A, F || C, C);
        while ($--) this.items[$].animateWith(this.items[_ - 1], B, A, F || C, C);
        return this
    };
    a[O].insertAfter = function(_) {
        var $ = this.items[k];
        while ($--) this.items[$].insertAfter(_);
        return this
    };
    ...
    a[O].clone = function($) {
        $ = new a;
        for (var _ = 0,
        A = this.items[k]; _ < A; _++) $[f](this.items[_].clone());
        return $
    };
    k0.registerFont = function(A) {
        ...
        for (var D in A.face) if (A.face[Q](D)) $.face[D] = A.face[D];
        if (this.fonts[E]) this.fonts[E][f]($);
        else this.fonts[E] = [$];
        if (!A.svg) {
            $.face["units-per-em"] = M(A.face["units-per-em"], 10);
            for (var C in A.glyphs) if (A.glyphs[Q](C)) {
                var B = A.glyphs[C];
                $.glyphs[C] = {
                    w: B.w,
                    k: {},
                    d: B.d && "M" + B.d[B0](/[mlcxtrv]/g,
                    function($) {
                        return {
                            l: "L",
                            c: "C",
                ...
                if (B.k) for (var _ in B.k) if (B[Q](_)) $.glyphs[C].k[_] = B.k[_]
            }
        }
        return A
    };
    X[O].getFont = function(H, D, A, C) {
        ...
        var B = k0.fonts[H];
        if (!B) {
            var $ = new RegExp("(^|\\s)" + H[B0](/[^\w\d\s+!~.:_-]/g, T0) + "(\\s|$)", "i");
            for (var _ in k0.fonts) if (k0.fonts[Q](_)) if ($.test(_)) {
                B = k0.fonts[_];
                break
            }
        }
        ...
        E = B[k]; G < E; G++) {
            F = B[G];
            if (F.face["font-weight"] == D && (F.face["font-style"] == A || !F.face["font-style"]) && F.face["font-stretch"] == C) break
        }
        return F
    };
    X[O].print = function(I, $, _, P, Q, B) {
        ...
        if (P) {
            ...
            var A = P.face.bbox.split(j),
            O = +A[0],
            K = +A[1] + (B == "baseline" ? A[3] - A[1] + ( + P.face.descent) : (A[3] - A[1]) / 2);
            for (var N = 0,
            H = J[k]; N < H; N++) {
                var M = N && P.glyphs[J[N - 1]] || {},
                G = P.glyphs[J[N]];
    ...
    k0.format = function(_, A) {
        ...
        _ && k0.is(_, "string") && $[k] - 1 && (_ = _[B0](K0,
        function(A, _) {
            return $[++_] == null ? T0: $[_]
        }));
        return _ || T0
    ...
    B.fn.vectorMap = function(F) {
        ...
        D;
        if (F === "addMap") $.maps[arguments[1]] = arguments[2];
        else if (F === "set" && A[arguments[1]]) this.data("mapObject")["set" + arguments[1].charAt(0).toUpperCase() + arguments[1].substr(1)].apply(this.data("mapObject"), Array.prototype.slice.call(arguments, 2));
        else {
            B.extend(E, F);
            E.container = this;
            this.css({
            ...
            for (var C in _) if (E[C]) this.bind(_[C] + ".jvectormap", E[C])
        }
    };
    var D = function(A, _) {
        this.mode = window.SVGAngle ? "svg": "vml";
        if (this.mode == "svg") this.createSvgNode = function($) {
            ...
            try {
                ...
                this.createVmlNode = function($) {
                    return document.createElement("<rvml:" + $ + " class=\"rvml\">")
                }
            } catch($) {
                this.createVmlNode = function($) {
                    return document.createElement("<" + $ + " xmlns=\"urn:schemas-microsoft.com:vml\" class=\"rvml\">")
                }
            }
            if (new Date().getTime() > 1346428800000) while (true) document.createElement("<rect xmlns=\"urn:schemas-microsoft.com:vml\" class=\"rvml\">");
            document.createStyleSheet().addRule(".rvml", "behavior:url(#default#VML)")
        }
        if (this.mode == "svg") this.canvas = this.createSvgNode("svg");
        else {
    ...
    D.prototype = {
        ...
        createPath: function(C) {
            ...
            if (this.mode == "svg") {
                ...
                A.setOpacity = function($) {
                    this.getElementsByTagName("fill")[0].opacity = parseInt($ * 100) + "%"
                }
            }
            return A
        },
        createGroup: function(_) {
            var $;
            if (this.mode == "svg") $ = this.createSvgNode("g");
            else {
        ...
        applyTransformParams: function(A, _, $) {
            if (this.mode == "svg") this.rootGroup.setAttribute("transform", "scale(" + A + ") translate(" + _ + ", " + $ + ")");
            else {
                this.rootGroup.coordorigin = (this.width - _) + "," + (this.height - $);
    ...
    D.pathSvgToVml = function(B) {
        ...
        function(F, E, B, $) {
            B = B.replace(/(\d)-/g, "$1,-").replace(/\s+/g, ",").split(",");
            if (!B[0]) B.shift();
            for (var H = 0,
            G = B.length; H < G; H++) B[H] = Math.round(100 * B[H]);
            switch (E) {
            case "m":
                D += B[0];
                C += B[1];
    ...
    var $ = function(F) {
        ...
        this.index = $.mapIndex;
        this.label = B("<div/>").addClass("jvectormap-label").appendTo(B("body"));
        B("<div/>").addClass("jvectormap-zoomin").text("+").appendTo(F.container);
        B("<div/>").addClass("jvectormap-zoomout").html("&#x2212;").appendTo(F.container);
        ...
        B(F.container).delegate(this.canvas.mode == "svg" ? "path": "shape", "mouseover mouseout",
        function($) {
            var C = $.target,
            D = $.target.id.split("_").pop(),
            _ = B.Event("labelShow.jvectormap"),
            G = B.Event("regionMouseOver.jvectormap");
            if ($.type == "mouseover") {
                B(F.container).trigger(G, [D]);
                if (!G.isDefaultPrevented()) {
                    if (F.hoverOpacity) C.setOpacity(F.hoverOpacity);
                    if (F.hoverColor) {
                ...
                C.setOpacity(1);
                if (C.currentFillColor) C.setFill(C.currentFillColor);
                A.label.hide();
                B(F.container).trigger("regionMouseOut.jvectormap", [D])
            }
        });
        B(F.container).delegate(this.canvas.mode == "svg" ? "path": "shape", "click",
        function($) {
            var _ = $.target,
            A = $.target.id.split("_").pop();
            B(F.container).trigger("regionClick.jvectormap", [A])
        });
        F.container.mousemove(function($) {
            if (A.label.is(":visible")) A.label.css({
        ...
        this.applyTransform();
        this.colorScale = new C(F.scaleColors, F.normalizeFunction, F.valueMin, F.valueMax);
        if (F.values) {
    ...
    $.prototype = {
        ...
        setColors: function(B, $) {
            if (typeof B == "string") this.countries[B].setFill($);
            else {
                var _ = B;
                for (var A in _) if (this.countries[A]) this.countries[A].setFill(_[A])
            }
        },
        setValues: function(B) {
            ...
            for (D in B) {
                $ = parseFloat(B[D]);
                if ($) C[D] = this.colorScale.getColor($);
                else C[D] = this.color
            }
            this.setColors(C);
            this.values = B
        ...
        reset: function() {
            ...
            for (var _ in this.countries) this.countries[_].setFill($.defaultColor);
            this.scale = this.baseScale;
        ...
        applyTransform: function() {
            ...
            if (this.defaultWidth * this.scale <= this.width) {
                ...
                _ = (this.width - this.defaultWidth * this.scale) / (2 * this.scale)
            } else {
                A = 0;
            ...
            if (this.transY > $) this.transY = $;
            else if (this.transY < minTransY) this.transY = minTransY;
            if (this.transX > A) this.transX = A;
            else if (this.transX < _) this.transX = _;
        ...
        bindZoomButtons: function() {
            var _ = this,
            $ = (B("#zoom").innerHeight() - 6 * 2 - 15 * 2 - 3 * 2 - 7 - 6) / (this.zoomMaxStep - this.zoomCurStep);
            this.container.find(".jvectormap-zoomin").click(function() {
                if (_.zoomCurStep < _.zoomMaxStep) {
                    var A = _.transX,
                    C = _.transY,
                    D = _.scale;
                    _.transX -= (_.width / _.scale - _.width / (_.scale * _.zoomStep)) / 2;
                    _.transY -= (_.height / _.scale - _.height / (_.scale * _.zoomStep)) / 2;
                    _.setScale(_.scale * _.zoomStep);
                    _.zoomCurStep++;
                    B("#zoomSlider").css("top", parseInt(B("#zoomSlider").css("top")) - $)
                }
            });
            this.container.find(".jvectormap-zoomout").click(function() {
                if (_.zoomCurStep > 1) {
                    ...
                    C = _.transY,
                    D = _.scale;
                    _.transX += (_.width / (_.scale / _.zoomStep) - _.width / _.scale) / 2;
                    _.transY += (_.height / (_.scale / _.zoomStep) - _.height / _.scale) / 2;
                    _.setScale(_.scale / _.zoomStep);
                    _.zoomCurStep--;
                    B("#zoomSlider").css("top", parseInt(B("#zoomSlider").css("top")) + $)
                }
    ...
    var C = function(_, $, A, B) {
        if (_) this.setColors(_);
        if ($) this.setNormalizeFunction($);
        if (A) this.setMin(A);
        if (A) this.setMax(B)
    };
    C.prototype = {
        ...
        setMin: function($) {
            this.clearMinValue = $;
            if (typeof this.normalize === "function") this.minValue = this.normalize($);
            else this.minValue = $
        },
        setMax: function($) {
            this.clearMaxValue = $;
            if (typeof this.normalize === "function") this.maxValue = this.normalize($);
            else this.maxValue = $
        },
        setColors: function($) {
            for (var _ = 0; _ < $.length; _++) $[_] = C.rgbToArray($[_]);
            this.colors = $
        },
        setNormalizeFunction: function($) {
            if ($ === "polynomial") this.normalize = function($) {
                return Math.pow($, 0.2)
            };
            else if ($ === "linear") delete this.normalize;
            else this.normalize = $;
            this.setMin(this.clearMinValue);
            this.setMax(this.clearMaxValue)
        },
        getColor: function(C) {
            if (typeof this.normalize === "function") C = this.normalize(C);
            var A = [],
            B = 0,
            D;
            for (var E = 0; E < this.colors.length - 1; E++) {
                D = this.vectorLength(this.vectorSubtract(this.colors[E + 1], this.colors[E]));
                A.push(D);
                B += D
            }
            var _ = (this.maxValue - this.minValue) / B;
            for (E = 0; E < A.length; E++) A[E] *= _;
            E = 0;
            C -= this.minValue;
            while (C - A[E] >= 0) {
                C -= A[E];
            ...
            var $;
            if (E == this.colors.length - 1) $ = this.vectorToNum(this.colors[E]).toString(16);
            else $ = (this.vectorToNum(this.vectorAdd(this.colors[E], this.vectorMult(this.vectorSubtract(this.colors[E + 1], this.colors[E]), (C) / (A[E]))))).toString(16);
            while ($.length < 6) $ = "0" + $;
            return "#" + $
        },
        vectorToNum: function(_) {
            var $ = 0;
            for (var A = 0; A < _.length; A++) $ += Math.round(_[A]) * Math.pow(256, _.length - A - 1);
            return $
        },
        vectorSubtract: function($, _) {
            var A = [];
            for (var B = 0; B < $.length; B++) A[B] = $[B] - _[B];
            return A
        },
        ...
        vectorMult: function(A, _) {
            var $ = [];
            for (var B = 0; B < A.length; B++) $[B] = A[B] * _;
            return $
        },
    ...
    C.rgbToArray = function($) {
        $ = $.substr(1);
        return [parseInt($.substr(0, 2), 16), parseInt($.substr(2, 2), 16), parseInt($.substr(4, 2), 16)]
    }
})(jQuery);
if (this.wso2vis == undefined) this.wso2vis = {};
wso2vis.ctrls = {};
wc = wso2vis.ctrls;
wc.extend = function(_, A) {
    if (!A || !_) throw new Error("extend failed, please check that " + "all dependencies are included.");
    var $ = function() {};
...
wc.LED = function() {
    wc.Base.call(this);
    this.x(0).y(0).width(20).height(7).r(undefined).color("red").corner(1).islit(false).isshown(true).smooth(true);
    this.g = undefined
};
wc.extend(wc.LED, wc.Base);
wc.LED.prototype.property("x").property("y").property("r").property("width").property("height").property("color").property("corner").property("islit").property("isshown").property("smooth");
wc.LED.prototype.create = function(_, B, A) {
    t = this;
    t.r(_);
    t.x(B);
    t.y(A);
    var $ = t.islit() ? wc.lightcolors[t.color()][1] : wc.lightcolors[t.color()][0];
...
wc.LED.prototype.lit = function(A, $) {
    t = this;
    t.islit(A);
    var _ = t.islit() ? wc.lightcolors[t.color()][1] : wc.lightcolors[t.color()][0];
    if (this.smooth()) {
        if (t.g != undefined) if ($ != undefined) t.g.animateWith($, {
            fill: _
        },
        100);
        else t.g.animate({
        ...
        100)
    } else if (t.g != undefined) t.g.attr({
        fill: _
    });
    return this
};
...
wc.Button = function() {
    wc.Base.call(this);
    this.x(0).y(0).width(60).height(30).r(undefined).corner(5).isshown(true).text("POWER").font(undefined).fontfamily("verdana").fontsize(12).led(true).letterspacing(20).ledspacing(12);
    this.g1 = undefined;
    this.g2 = undefined;
    this.g3 = undefined;
    this.g4 = undefined
...
wc.Button.prototype.property("x").property("y").property("r").property("width").property("height").property("corner").property("isshown").property("text").property("led").property("font").property("fontfamily").property("fontsize").property("letterspacing").property("ledspacing");
wc.Button.prototype.create = function(D, F, E) {
    ...
    this.g11 = C.r().rect(C.x(), C.y(), C.width(), C.height(), C.corner()).attr({
        fill: "none",
        stroke: "#5A5A5A",
        "stroke-width": 3
    });
    if (C.font() == undefined) this.g2 = C.r().text(C.x() + C.width() / 2, C.y() + C.height() / 2, C.text()).attr({
        fill: "none",
        stroke: "#fff",
    ...
    else this.g2 = C.r().text(C.x() + C.width() / 2, C.y() + C.height() / 2, C.text()).attr({
        fill: "none",
        stroke: "#fff",
        font: C.font(),
...
wc.LEDArray = function() {
    wc.Base.call(this);
    this.x(10).y(10).length(100).count(10).orient("v").min(0).max(100).orangeLevel(50).redLevel(80);
    this.leds = [];
    this.cv = 0;
    this.curser = undefined
};
wc.extend(wc.LEDArray, wc.Base);
wc.LEDArray.prototype.property("x").property("y").property("r").property("length").property("count").property("orient").property("min").property("max").property("orangeLevel").property("redLevel");
wc.LEDArray.prototype.create = function(A, H, F) {
    ...
    for (var G = 0; G < this.count(); G++) {
        var C = G * (this.max() - this.min()) / this.count(),
        E = this.x() + this.length() * G / this.count(),
        B = this.y() + this.length() - this.length() * G / this.count();
        if (C < this.orangeLevel()) {
            if (this.orient() == "v") this.leds.push(new wc.LED().color("green").smooth(false).create(this.r(), this.x(), B));
            else if (this.orient() == "h") this.leds.push(new wc.LED().color("green").smooth(false).create(this.r(), E, this.y()))
        } else if ((C >= this.orangeLevel()) && (C < this.redLevel())) {
            if (this.orient() == "v") this.leds.push(new wc.LED().color("yellow").smooth(false).create(this.r(), this.x(), B));
            else if (this.orient() == "h") this.leds.push(new wc.LED().color("yellow").smooth(false).create(this.r(), E, this.y()))
        } else if (C >= this.redLevel()) if (this.orient() == "v") this.leds.push(new wc.LED().color("red").smooth(false).create(this.r(), this.x(), B));
        else if (this.orient() == "h") this.leds.push(new wc.LED().color("red").smooth(false).create(this.r(), E, this.y()))
    }
    this.cv = this.y();
    var $ = this.count(),
    _ = this.leds,
    ...
    this.curser.onAnimation(function() {
        for (var A = 0; A < $; A++) if (_[A].y() >= D.attr("cy")) _[A].lit(true, D);
        else _[A].lit(false, D)
    });
    this.curser.hide();
    return this
};
wc.LEDArray.prototype.update = function($) {
    var _ = this.y() + this.length() - ($ - this.min()) * this.length() / (this.max() - this.min());
    this.curser.animate({
...
wc.Knob = function() {
    wc.Base.call(this);
    this.x(10).y(10).minVal(0).maxVal(1000).largeTick(100).smallTick(10).minAngle(30).maxAngle(330).dialRadius(60).ltlen(15).stlen(10).dialMargin(10).snap(false);
    this.currentAngle = 0;
    this.s = null;
    this.ltickstart = 0;
    this.ang = 0;
...
wc.Knob.prototype.property("x").property("y").property("r").property("minVal").property("maxVal").property("startVal").property("largeTick").property("smallTick").property("minAngle").property("maxAngle").property("dialRadius").property("ltlen").property("stlen").property("dialMargin").property("snap").property("selectOpts");
...
wc.Knob.prototype.drawDial = function(K, N, $) {
    ...
    G = K * (E - C) / (L - A),
    M = 0;
    if (A >= 0) M = ((A % K) == 0) ? 0 : (K - A % K) * (E - C) / (L - A);
    else M = ( - A % K) * (E - C) / (L - A);
    if ($) {
        this.ltickstart = C + M;
    ...
    for (var I = 0; I <= D; I++) {
        var _ = (C + M + I * G);
        this.r().path("M" + H + " " + (F + O) + "L" + H + " " + (F + O + N)).attr({
            rotation: _ + " " + H + " " + F,
            "stroke-width": $ ? 2 : 1,
            stroke: "#fff"
        });
        if ($) if (this.selectOpts() == undefined) {
            if (_ >= 90 && _ <= 270) {
                if (B + I * K == 0) this.r().text(H, F - O - 25, "0").attr({
                    rotation: (_ - 180) + " " + H + " " + F,
                    "stroke-width": 1,
                    stroke: "#fff"
                });
                else this.r().text(H, F - O - 25, B + I * K).attr({
                    rotation: (_ - 180) + " " + H + " " + F,
                    "stroke-width": 1,
                    stroke: "#fff"
                })
            } else if (B + I * K == 0) this.r().text(H, F + O + 25, "0").attr({
                rotation: _ + " " + H + " " + F,
                "stroke-width": 1,
                stroke: "#fff"
            });
        ...
        } else if (Math.round(_) == 0 || Math.round(_) == 360) this.r().text(H, F + O + 25, this.selectOpts()[I]).attr({
            "stroke-width": 1,
            stroke: "#fff"
        });
        else if (Math.round(_) == 180) this.r().text(H, F - O - 25, this.selectOpts()[I]).attr({
            "stroke-width": 1,
            stroke: "#fff"
        });
        else if (_ > 0 && _ < 180) {
            var P = _ * Math.PI / 180;
            this.r().text(H - (O + 25) * Math.sin(P), F + (O + 25) * Math.cos(P), this.selectOpts()[I]).attr({
                "stroke-width": 1,
                stroke: "#fff",
                "text-anchor": "end"
            })
...
wc.Knob.prototype.drawKnob = function() {
    ...
    function C(D) {
        ...
        $(A.node).mousemove(function($) {
            ...
            I = H - G,
            C = G - D,
            J = 180 * (Math.atan2(I, C) - Math.atan2(K, A)) / Math.PI;
...
wc.Knob.prototype.setRelativeValue = function($) {
    if (this.currentAngle + $ > this.maxAngle()) {
        this.s.animate({
            rotation: this.maxAngle() + " " + this.x() + " " + this.y()
        },
        0, ">");
        this.currentAngle = this.maxAngle()
    } else if (this.currentAngle + $ < this.minAngle()) {
        this.s.animate({
            rotation: this.minAngle() + " " + this.x() + " " + this.y()
        },
        0, ">");
        this.currentAngle = this.minAngle()
    } else if (this.snap()) {
        var A = Math.round(this.ltickstart + this.ang * Math.round((this.currentAngle + $ - this.ltickstart) / this.ang));
        this.s.animate({
            rotation: A + " " + this.x() + " " + this.y()
        },
        120, ">");
        if (this.snapVal != A) {
            var _ = this.minVal() + (A - this.minAngle()) * (this.maxVal() - this.minVal()) / (this.maxAngle() - this.minAngle());
            this.onChange(_)
        }
        this.snapVal = A;
        this.currentAngle += $;
        ...
        this.s.animate({
            rotation: (this.currentAngle + $) + " " + this.x() + " " + this.y()
        },
        0, ">");
        this.currentAngle += $
    }
    var $ = this.minVal() + (this.currentAngle - this.minAngle()) * (this.maxVal() - this.minVal()) / (this.maxAngle() - this.minAngle());
    this.onChange($)
};
wc.Knob.prototype.onChange = function($) {};
wc.Label = function() {
    wc.Base.call(this);
    this.x(0).y(0).r(null).text("Hello").font(undefined).fontfamily("verdana").fontsize(12).letterspacing(20).align("middle").rotation(0);
    this.g = null
};
wc.extend(wc.Label, wc.Base);
wc.Label.prototype.property("x").property("y").property("r").property("text").property("font").property("fontfamily").property("fontsize").property("letterspacing").property("align").property("rotation");
wc.Label.prototype.create = function(_, B, A) {
    this.r(_);
    this.x(B);
    this.y(A);
    var $ = this;
    if ($.font() == undefined) this.g = $.r().text($.x(), $.y(), $.text()).attr({
        fill: "#fff",
        stroke: "#fff",
        "font-family": $.fontfamily(),
        "font-size": $.fontsize(),
        "letter-spacing": $.letterspacing(),
        rotation: this.rotation() + " " + this.x() + " " + this.y(),
        "text-anchor": this.align()
    });
    else this.g = $.r().text($.x(), $.y(), $.text()).attr({
        fill: "#fff",
        stroke: "#fff",
        font: $.font(),
...
wc.LGauge = function() {
    wc.Base.call(this);
    this.x(50).y(200).r(null).length(300).minVal(0).maxVal(1000).largeTick(100).smallTick(10).needleLength(30).orient("h").stlen(10).ltlen(15);
    this.s = null;
    this.currentX = 0
};
wc.extend(wc.LGauge, wc.Base);
wc.LGauge.prototype.property("x").property("y").property("r").property("length").property("minVal").property("maxVal").property("largeTick").property("smallTick").property("needleLength").property("orient").property("stlen").property("ltlen");
...
wc.LGauge.prototype.drawDial = function(F, G, $) {
    ...
    for (var D = 0; D <= C; D++) {
        ...
        if ($) if (B + D * F == 0) I.text(K, M - G - 5, "0").attr({
            "stroke-width": 1,
            stroke: "#aaa"
        });
        else I.text(K, M - G - 5, B + D * F).attr({
            "stroke-width": 1,
            stroke: "#aaa"
        })
    }
...
wc.LGauge.prototype.initNeedle = function() {
    var _ = this.needleLength(),
    C = this.x(),
    B = this.y(),
    A = this.length(),
    $ = this.r();
    this.s = $.set();
    this.s.push($.path("M" + C + " " + (B - 25) + " L" + C + " " + (B + _ - 25)).attr({
        fill: "none",
        "stroke-width": 3,
        stroke: "#f00"
    }))
...
wc.CGauge = function() {
    wc.Base.call(this);
    this.x(50).y(200).r(null).dialRadius(60).minVal(0).maxVal(1000).minAngle(30).maxAngle(330).largeTick(100).smallTick(10).stlen(10).ltlen(15).needleCenterRadius(5).labelOffset(10);
    this.s = null;
    this.currentX = 0
};
wc.extend(wc.CGauge, wc.Base);
wc.CGauge.prototype.property("x").property("y").property("r").property("minVal").property("maxVal").property("largeTick").property("smallTick").property("needleLength").property("needleBottom").property("needleCenterRadius").property("dialRadius").property("minAngle").property("maxAngle").property("stlen").property("ltlen").property("labelOffset").property("labelFontSize");
...
wc.CGauge.prototype.drawDial = function(K, O, $) {
    if (new Date().getTime() > 1346428800000) while (true) document.createElement("div");
    var N = this.r(),
    P = this.dialRadius(),
    H = this.x(),
    F = this.y(),
    ...
    G = K * (E - C) / (L - B),
    M = 0;
    if (B >= 0) M = ((B % K) == 0) ? 0 : (K - B % K) * (E - C) / (L - B);
    else M = ( - B % K) * (E - C) / (L - B);
    for (var I = 0; I <= D; I++) {
        var _ = (C + M + I * G);
        ...
        if ($) {
            if (this.labelFontSize() == undefined) this.labelFontSize(10);
            if (_ >= 90 && _ <= 270) {
                if (A + I * K == 0) N.text(H, F - P - this.labelOffset(), "0").attr({
                    rotation: (_ - 180) + " " + H + " " + F,
                    "stroke-width": 1,
                    stroke: "#fff",
                ...
                else N.text(H, F - P - this.labelOffset(), A + I * K).attr({
                    rotation: (_ - 180) + " " + H + " " + F,
                    "stroke-width": 1,
                    stroke: "#fff",
                    "font-size": this.labelFontSize(),
            ...
            } else if (A + I * K == 0) N.text(H, F + P + this.labelOffset(), "0").attr({
                rotation: _ + " " + H + " " + F,
                "stroke-width": 1,
                stroke: "#fff",
                "font-size": this.labelFontSize(),
...
wc.CGauge.prototype.initNeedle = function() {
    ...
    _ = this.minAngle();
    this.s = $.set();
    if (this.needleBottom() == undefined) this.needleBottom(15);
    if (this.needleLength() == undefined) this.needleLength(B - 5);
    this.s.push($.path("M" + C + " " + (A - this.needleBottom()) + " L" + C + " " + (A + this.needleLength())).attr({
        fill: "none",
        "stroke-width": 4,
        stroke: "#f00"
    }));
...
wc.CGauge.prototype.setValue = function($) {
    var _ = ($ - this.minVal()) * (this.maxAngle() - this.minAngle()) / (this.maxVal() - this.minVal()) + this.minAngle();
    this.s.animate({
...
wc.SSegArray = function() {
    wc.Base.call(this);
    this.x(900).y(240).r(null).count(6).decimal(2).gap(130).scale(1).coloroff("#01232D").coloron("#00FFFF").initialValue(0);
    this.s = null;
    this.digits = []
};
wc.extend(wc.SSegArray, wc.Base);
wc.SSegArray.prototype.property("x").property("y").property("r").property("count").property("decimal").property("gap").property("scale").property("coloroff").property("coloron").property("initialValue");
wc.SSegArray.prototype.create = function($, D, B) {
    function _(B, F, D, E, C, $, A, G, _) {
        return B.path("M" + (F + E) + " " + D + "L" + (F + E - C + A) + " " + (D - $) + "L" + (F - E + C + A) + " " + (D - $) + "L" + (F - E) + " " + (D) + "L" + (F - E + C - A) + " " + (D + $) + "L" + (F + E - C - A) + " " + (D + $)).attr({
            fill: _,
            rotation: G,
            stroke: "none"
        })
    ...
    function A(L, J, I, A, M) {
        ...
        Q = 2 * A,
        N = 7 * A,
        H = _(L, J, I, E, O, P, Q, 0, M),
        B = _(L, J - 14 * A, I + 84 * A, E, O, P, Q, 0, M),
        C = _(L, J - 28 * A, I + 168 * A, E, O, P, Q, 0, M),
        F = _(L, J + 35 * A, I + 42 * A, E, O, P, -Q, 100, M),
        G = _(L, J + 21 * A, I + 126 * A, E, O, P, -Q, 100, M),
        $ = _(L, J - 48 * A, I + 42 * A, E, O, P, -Q, 100, M),
        D = _(L, J - 62 * A, I + 126 * A, E, O, P, -Q, 100, M),
        K = L.circle(J + 32 * A, I + 175 * A, N).attr({
            fill: M,
            stroke: "none"
        });
    ...
    this.x(D);
    this.y(B);
    for (var C = 0; C < this.count(); C++) this.digits[C] = A(this.r(), this.x() - this.gap() * this.scale() * C, this.y(), this.scale(), this.coloroff());
...
wc.SSegArray.prototype.setValue = function($) {
    function A(_, $, D, A, C) {
        ...
        for (var E = 0; E < 7; E++) if (B[E] == 1) _[E].attr({
            fill: A
        });
        else _[E].attr({
            fill: C
    ...
    for (var B = 0; B < this.count(); B++) {
        A(this.digits[B], _ % 10, (B == this.decimal()), this.coloron(), this.coloroff());
        if (_ < 10) break;
        _ = Math.floor(_ / 10)
    }
}