(function (b) {
	...
	a.rect = function (p, m) {
		...
		/************************/
		this.statesxml = function (obj) {
			var pros = "";
			statesxml = statesxml + "{rect:'" + obj + "',type:'" + E.type + "',x:"
				 + Math.round(t.attr("x")) + ", y:" + Math.round(t.attr("y")) + ", width:"
				 + Math.round(t.attr("width")) + ",height:" + Math.round(t.attr("height"));
			for (var o in E.props) {
			  E.props[o].value=  E.props[o].value.replace(/\"/ig,"\\\"");
				statesxml += "," + o + ":'" + E.props[o].value + "'";
			}
			statesxml = statesxml + "},";
			return statesxml;
		}
	...
	a.path = function (q, n, u, e) {
		...
		/***************连接的修改***************/
		this.transitionxml = function () {
			var dt = x.toxml();
			if (dt == '') {
				transitionxml = transitionxml + "{from:'" + k.getId() + "',to:'" + s.getId() + "',name:'"
					 + f.attr("text") + "',x:" + Math.round(h.x) + ",y:" + Math.round(h.y) + ",";
			} else {
				dt = eval('(' + dt + ')');
				transitionxml = transitionxml + "{from:'" + k.getId() + "',to:'" + s.getId() + "',name:'"
					 + f.attr("text") + "',dots:'" + dt.x + "," + dt.y + "',x:" + Math.round(h.x) + ",y:" + Math.round(h.y) + ",";
			}
			for (var o in B.props) {
				transitionxml += o + ":'" + B.props[o].value + "',"
			}
			if (transitionxml.substring(transitionxml.length - 1, transitionxml.length) == ",") {
				transitionxml = transitionxml.substring(0, transitionxml.length - 1)
			}
			transitionxml = transitionxml + "},";
		...
		if (a.config.editable) {
			...
			b("#myflow_save").click(function () {
				...
				for (var c in q) {
					if (q[c]) {
						/*******控件***********/
						bbs = q[c].statesxml(q[c].getId());
						i += q[c].getId() + ":" + q[c].toJson() + ",";
						
					}
				}
				if (bbs.substring(bbs.length - 1, bbs.length) == ",") {
				...
				for (var c in g) {
					if (g[c]) {
						...
						transitionxml = g[c].transitionxml();
				...
				for (var c in a.config.props.props) {
					...
					processxml = processxml + c + ":'" + a.config.props.props[c].value + "',";
					
				}
				
				if (processxml.substring(processxml.length - 1, processxml.length) == ",") {
					processxml = processxml.substring(0, processxml.length - 1)
				}
