...
Ext.extend(Ext.ux.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
    renderElements : function(n, a, targetNode, bulkRender) {
		...
		var checkImg = cb? '<img style="margin-bottom:-3px;" src="'+tree.imagePath+'/'+ n.attributes.checked + '.gif">': '';
		var href = a.href ? a.href : Ext.isGecko ? "" : "#";
		var buf = ['<li class="x-tree-node"><div ext:tree-node-id="',n.id,
				'" class="x-tree-node-el x-tree-node-leaf x-unselectable ',a.cls,
				'" unselectable="on">','<span class="x-tree-node-indent">',this.indentMarkup,
				"</span>",'<img src="',this.emptyIcon,'" class="x-tree-ec-icon x-tree-elbow" />',
				'<img src="',a.icon || this.emptyIcon,'" class="x-tree-node-icon',
				(a.icon ? " x-tree-node-inline-icon" : ""),(a.iconCls ? " " + a.iconCls : ""),
				'" unselectable="on" />',checkImg,'<a hidefocus="on" class="x-tree-node-anchor" href="',
    ...
    check : function(checked,childChecked,parentChecked) {
	    if ("undefined" == typeof childChecked) childChecked=true;
		if ("undefined" == typeof parentChecked) parentChecked=true;
		var n = this.node;
    ...
    parentCheck : function(node) {
		...
		while ((currentNode = currentNode.parentNode) != null) {
			...
			Ext.each(currentNode.childNodes, function(child) {// 如果子节点全部checked
				// ==
				// 'all',父节点也为全选,否则为半选
				if (child.attributes.checked == 'all') sel++;
    ...
    setNodeIcon : function(n) {
		...
		n.getUI().checkbox.src = n.getOwnerTree().imagePath +"/"+ n.attributes.checked+ '.gif';
    },
    // private
    ...
    */
    getChecked : function(a, startNode) {
		...
		var f = function() {
			...
			} else if (this.attributes.checked == 'part') {
				return true;
			} else if (this.isRoot){
			    return true
			} else{
